...
// ============= TECH ORBIT DATA =============
const STACK_TOP = ["Figma", "React", "C++", "SQL", "Python", "Node.js", "JavaScript"];
const STACK_TOP2 = ["Next.js", "AWS", "MongoDB"];
const RINGS = [
...
const ROLES = [
  {
    icon: IconRocket,
    role: "Web Developer",
    org: "Huban Technologies",
    period: "May 2025- October 2025",
    desc: " Built responsive web applications with HTML, CSS and JavaScript, leveraging agile methodologies to iterate on business requirements and deliver features in short cycles– Implemented RESTful APIs for effective frontend-backend communication, enabling business process automation and scalable technology integration– Created and optimized RDBMS schemas, tables and SQL queries for efficient data management, applying data structure principles to improve query performance– Conducted QA testing, unit testing, debugging and API validation using Postman, ensuring reliable endpoints and reducing application defects during development cycles– Tech Stack: HTML, CSS, JavaScript, Node.js, MySQL, REST APIs, Postman, Git, GitHub",
  },
...
const PROJECTS = [
  {
    ...
    desc: "Developed and deployed fraud detection models using statistical and machine learning techniques to identify anomalous transactions at scale. Applied SMOTE to address class imbalance, improving model reliability and business risk analysis. Generated actionable financial risk insights from complex datasets, increasing fraud detection efficiency by 15%. Enhanced model precision by 12% through SMOTE-based balancing, feature engineering, and model optimization.",
    techs: ["Python", "Scikit-Learn", "Pandas", "NumPy", "SMOTE"],
    link: "https://frauddetection12.streamlit.app/",
...

{
  ...
  label: "Featured Project",
  desc: "Developed a real-time Network Intrusion Detection System using Python, Flask, and Machine Learning to classify network traffic as normal or malicious. Built and trained classification models on network packet features to identify suspicious activities and enhance threat detection. Implemented live packet capture and feature extraction using Scapy for real-time traffic analysis. Designed an interactive dashboard for monitoring network events and visualizing intrusion alerts.",
  techs: [
    "Python",
    "Flask",
    "Scikit-Learn",
...
const SOCIALS = [
  {
    name: "GitHub",
    url: "https://github.com/ayush234678",
    icon: "github",
  },
  {
    name: "LinkedIn",
    url: "https://www.linkedin.com/in/ayush-anand-098659278/",
    icon: "linkedin",
  },
  {
    name: "LeetCode",
    url: "https://leetcode.com/u/ayushanand1205/",
    icon: "leetcode",
  },
];
    ...
    <div className="landing-page">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        .dh-nav a { color: ${TEXT}; text-decoration: none; opacity: 0.85; position: relative; padding: 8px 0 4px; display: inline-block; min-height: 36px; display: flex; align-items: center; }
        ...
        @media (max-width: 760px) {
          ...
          .dh-mast { padding: 18px 24px; flex-wrap: wrap; }
          .dh-mark { font-size: 18px; }
          .dh-nav { gap: 16px; font-size: 12px; width: 100%; margin-top: 12px; flex-wrap: wrap; }
          .dh-hero { flex-direction: column; align-items: flex-start; gap: 28px; padding: 40px 0 20px; }
          .dh-avatar-wrap { width: 180px; height: 180px; }
          .dh-glow { inset: -30px; }
          .dh-intro-tag { position: static; margin-top: 12px; margin-bottom: 45px; display: block; font-size: 12px; }
          .dh-intro-tag::before { display: none; }
          .dh-headline { font-size: clamp(28px, 6vw, 34px); }
          .dh-subline { font-size: 14px; max-width: 100%; }
          .dh-typewriter-row { font-size: 18px; }
          .dh-bio-row { flex-wrap: wrap; font-size: 13px; }

          /* ===== TECH ORBIT MOBILE ===== */
          .orbit-section { padding: 56px 16px 80px; }
          .orbit-head { margin-bottom: 40px; }
          .orbit-headline { font-size: clamp(20px, 5vw, 26px); }
          .orbit-top-row { gap: 10px; margin-bottom: 8px; }
          .orbit-top-icon { width: 36px; height: 36px; }
          .orbit-lines { width: 90vw; height: 140px; }
          .orbit-core-wrap { height: 280px; margin-top: -20px; }
          .orbit-glow { width: 380px; height: 380px; }
          .orbit-core { width: 80px; height: 80px; }
          .orbit-core-mark { font-size: 24px; }

          /* ===== WORK EXPERIENCE MOBILE ===== */
          .we-section { padding: 56px 16px 80px; }
          .we-container { padding: 0; }
          .we-title { font-size: clamp(24px, 6vw, 32px); }
          .we-grid { grid-template-columns: 1fr; gap: 16px; }
          .card { flex-direction: column; gap: 12px; padding: 20px 16px; }
          .card-icon-wrap { width: 48px; height: 48px; }
          .card-role { font-size: 16px; }
          .card-desc { font-size: 13px; margin: 8px 0 16px; }
          .card-btn { padding: 8px 12px; font-size: 10px; min-height: 36px; }

          /* ===== FEATURED PROJECTS MOBILE ===== */
          .fp-section { padding: 56px 16px 80px; }
          .fp-container { padding: 0; }
          .fp-title { font-size: clamp(24px, 6vw, 32px); }
          .fp-grid { grid-template-columns: 1fr; gap: 16px; }
          .project-card { padding: 20px 16px; grid-template-columns: 1fr; gap: 16px; }
          .project-title { font-size: 20px; }
          .project-desc { font-size: 13px; }
          .tech-badge { font-size: 10px; padding: 4px 8px; }
          .project-link-btn { padding: 8px 12px; font-size: 10px; min-height: 36px; }

          /* ===== CONTACT MOBILE ===== */
          .contact-section { padding: 56px 16px 60px; }
          .contact-container { padding: 0; }
          .contact-title { font-size: clamp(24px, 6vw, 32px); }
          .contact-email { font-size: 14px; margin-bottom: 32px; }
          .social-links { gap: 14px; }
          .social-icon { width: 44px; height: 44px; font-size: 18px; min-height: 44px; }
          ...
          .dh-about { padding-bottom: 60px; }
          .dh-about p { font-size: 15px; margin: 0 0 14px; }

          /* ===== GENERAL MOBILE ===== */
          .dh-divider { margin: 40px 0 32px; }
        }

        @media (max-width: 480px) {
          /* ===== EXTRA SMALL SCREENS ===== */
          .dh-container { padding: 0 16px; }
          .dh-mast { padding: 14px 16px; }
          .dh-mark { font-size: 16px; }
          .dh-nav { gap: 12px; font-size: 11px; }
          .dh-hero { gap: 20px; padding: 32px 0 16px; }
          .dh-avatar-wrap { width: 160px; height: 160px; }
          .dh-headline { font-size: 24px; }
          .dh-subline { font-size: 13px; }
          .dh-typewriter-row { font-size: 16px; }

          .orbit-section { padding: 40px 12px 60px; }
          .orbit-top-icon { width: 32px; height: 32px; }
          .orbit-core-wrap { height: 240px; }

          .we-section { padding: 40px 12px 60px; }
          .we-title { font-size: 22px; }
          .card { padding: 16px 12px; gap: 10px; }

          .fp-section { padding: 40px 12px 60px; }
          .fp-title { font-size: 22px; }
          .project-card { padding: 16px 12px; }

          .contact-section { padding: 40px 12px 50px; }
          ...
          <section className="dh-hero" id="home">
            ...
            <Reveal as="div" className="dh-hero-text" delay={120}>
              ...
              <h1 className="dh-headline">
                ...
                <br />
                <TerminalCursorWord word=", and turning data into insights." />.
              </h1>
